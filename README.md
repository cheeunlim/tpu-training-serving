# Cloud TPU 및 vLLM 실습
-   https://github.com/cheeunlim/tpu-training-serving
...
# Task 0. Vertex AI Workbench 환경 구성 및 Vertex AI Serving 예제 Prewarm
...
```
git clone https://github.com/cheeunlim/tpu-training-serving
```

아래의 노트북 파일을 열어 각각 첫번째 셀을 실행합니다.
...
```
serving/vertexai/handson-gpu.ipynb
```

```
serving/vertexai/handson-tpu.ipynb
```

첫째 셀을 실행한 후 두번째 셀을 선택한 후 Run -> Run Selected Cell and All Below 를 클릭하여 나머지 셀들을 모두 실행해 둡니다.
...
```
training/JAX_for_LLM_pretraining.ipynb
```
...
```
serving/python/vLLM_LoadTest.ipynb
```
...
Task 0에서 이미 서빙 엔드포인트를 배포했기 때문에, 가장 마지막 셀의 내용만 바꿔가며 테스트해 봅니다.
